...
export default class Bundler {
  ...
  public async bundle(chunks: Chunk[]): Promise<Chunk[]> {
    ...
    let bundledChunks = []
    const bundleTasks = this.bundlers.map(({ regexp, bundler: Bundler }) => {
      /**
       * 过滤需要打包的文件, 这里先判断文件类型
        ...
        if (chunk.type !== SCATTER) {
          if (Array.isArray(chunk.destination)) {
            for (let i = chunk.destination.length; i--; ) {
              const destination = chunk.destination[i]
      ...
      const bundler = new Bundler(bundleChunks, this.options)
    ...
    const transformTasks = bundledChunks.map(chunk => {
  ...
  public matchBundler(chunks): Bundler['bundlers'] {
    return this.bundlers.filter(({ regexp }) => {
      const index = chunks.findIndex(chunk => regexp.test(chunk.file))
      return index !== -1
    })
  }
}
