...
export default class OptionManager {
  ...
  public async resolve(options: Typings.ParcelOptions = {}): Promise<void> {
    ...
    const valid = this.checkRules(this.rules)
    if (valid !== true) {
    ...
    const wxProjConfFile = path.join(this.rootDir, './project.config.json')
    this.resolveWXProjConf(wxProjConfFile)

    this.appConfig = {}
    this.appConfigFile = ''

    const wxAppConfFile = path.join(this.miniprogramRoot, './app.json')
  ...
  public checkRules(rules: Typings.ParcelOptionRule[] = []): true | string {
    for (let i = rules.length; i--; ) {
      const rule = rules[i]
      const genMessage = () => {
        const tmpRule = Object.assign({}, rule) as any
  ...
  public resolveWXProjConf(file: string): void {
    if (!fs.existsSync(file)) {
      const message = `File ${file} is not found, please ensure ${file} is valid.`
    ...
    try {
      ...
      const message = `File ${file} is invalid json, please check the json corrected.\n${error}`
      throw new Error(message)
    }

    let { miniprogramRoot } = this.projectConfig
    if (miniprogramRoot) {
      const app = path.basename(this.outDir)
