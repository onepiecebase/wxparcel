...
export default class JSResolver extends Resolver {
  ...
  public convertFinallyState(source: string, { code, dependency, destination, required, ...props }) {
    ...
    if (extname === '' || /\.(jsx?|babel|es6)/.test(extname)) {
      ...
      const dependence = { dependency, destination, required, ...props }
    ...
    const dependencyDestination = this.convertAssetsDestination(dependency)
    const url = this.convertPublicPath(dependencyDestination)

    source = source.replace(new RegExp(escapeRegExp(code), 'ig'), `"${url}"`)
    const dependence = { dependency, destination: dependencyDestination, required, ...props }
    return [source, dependence]
  }
  ...
  public convertWorkerRelative(requested: string): string {
    ...
    if (workers !== workerFolder) {
      throw new Error(
        `Worker folder ${workerFolder} is not defined in app.config.json. Please check config with docs https://developers.weixin.qq.com/miniprogram/dev/framework/workers.html`
      )
    }
  ...
  public convertRelative(requested: string, relativePath: string): string {
     ...
     */
    try {
      const file = this.convertDependency(requested, relativePath)
