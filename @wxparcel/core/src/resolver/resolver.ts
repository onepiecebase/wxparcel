...
export default class Resolver {
  ...
  public revise(info: any[], pattern: RegExp | RegExp[], options: ReviseOptions = {}) {
    ...
    const { convertDependency, convertDestination, convertFinallyState } = options
    let code = source
    ...
    while (true) {
      ...
      const [all, ...required] = match
      const validRequired = required.find(item => typeof item !== 'undefined')
      ...
      if (findIndex(dependencies, props) === -1) {
        const destination = convertDestination(dependency as string, this.options)
  ...
  public convertAtRequired(required: string): string {
    ...
    if (required.charAt(0) === '@') {
      ...
      switch (required.charAt(0)) {
        ...
        case '/':
          const src = path.basename(srcDir)
  ...
  public convertDestination(file: string): string {
     ...
     */
    return file.indexOf(srcDir) !== -1 ? file.replace(srcDir, outDir) : file.replace(rootDir, outDir)
  }
   ...
   */
  public convertFinallyState(source: string, dependence: { [key: string]: any }) {
    const props = omit(dependence, 'code')
    return [source, props]
  }
  ...
  public convertAssetsDestination(file: string): string {
    ...
    const filename = basename + '.' + genFileSync(file) + extname
